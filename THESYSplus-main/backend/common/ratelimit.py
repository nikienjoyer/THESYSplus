...
Development Mode:
When DEBUG=True, ``rate_limit_per_ip`` and ``rate_limit_per_email`` relax
their limits to 20 requests/minute for local testing. Production limits
remain unchanged when DEBUG=False.

``rate_limit_per_email_on_failure`` (the login failure budget) is
deliberately NOT relaxed in DEBUG — brute-force protection on login stays
at its configured limit (5 failures / 15 minutes) in every environment.
"""
